...
const handleInput = async body => {
  let athleteInfo
  try {
    athleteInfo = await API.graphql(
      graphqlOperation(queries.getAthleteAll),
  ...
  switch (body.object_type) {
    case "activity":
      ...
      return
    case "athlete":
      await handleAthlete(body, athleteInfo.data.getAthleteByStravaId.items[0]);
      return
    default:
      return;
  }
...
const handleActivity = async (body, athleteInfo) => {
  switch (body.aspect_type) {
    ...
    case "update":
      try {
        let activity = await getActivity(body.object_id, athleteInfo.auth.access_token)
        let dbActivities = await API.graphql(graphqlOperation(queries.getActivityByStravaId), { strava_id: body.object_id })
...
const handleAthlete = async (body, athleteInfo) => {
  switch (body.aspect_type) {
    case "update":
      ...
      return
    case "delete":
      API.graphql(graphqlOperation(queries.deleteAthlete), { input: { id: body.object_id }})
      return;
    default:
      return;
  }
...
const getAthlete = async (auth) => {
  try {
    const athlete = await axios.post(
...
const refreshAuth = async athleteInfo => {
  const data = {
    ...
    refresh_token:
      athleteInfo.data.getAthleteByStravaId.items[0].auth.refresh_token,
    grant_type: "refresh_token"
  };
  let res = await axios.post("https://www.strava.com/oauth/token", data);
  let { athlete, expires_in, ...auth } = res.data;
  auth["id"] = athleteInfo.data.getAthleteByStravaId.items[0].auth.id;
