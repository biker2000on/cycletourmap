<template>
    ...
    <v-flex text-center>
      <h2>Summary Stats</h2>
      <div>
        <strong>Average Riding Speed:</strong>
        <br />
      ...
      <div>
        <strong>Average Speed w/ Stops:</strong>
        <br />
      ...
      <div>
        <strong>Total Riding Time</strong>
        <br />
        <h3>{{ (summaryStats.moving_time / 3600).toFixed(2) }} hrs</h3>
      </div>
      <div>
        <strong>Total Elapsed Time on Bike</strong>
        <br />
  ...
  computed: {
    ...
    summaryStats() {
      ...
      let totals = this.$store.state.activities.reduce((a, c) => {
        ...
        if (
          !a.day.includes(c.start_date_local.slice(0, 10)) &&
          !c.name.includes("Rest Day")
        ) {
    ...
    days() {
      ...
      let now = new Date();
      return Math.round(
        Math.abs((now.getTime() - start.getTime()) / (24 * 60 * 60 * 1000))
      );
    }
  }
};
